    ...
    <Page className="space-y-6">
      ...
      {canSales && shopId ? (
        <Card className="p-5 space-y-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <div className="text-sm font-semibold text-white">Sales summary</div>
              <div className="mt-1 text-xs text-slate-400">Filter by date range.</div>
            </div>
          ...
          {sales !== null ? (
            <div className="grid gap-3 sm:grid-cols-2">
              <Card className="p-4">
                <div className="text-xs uppercase tracking-wider text-slate-400">Total revenue</div>
                <div className="mt-2 text-2xl font-semibold text-white">₹{Number(sales.totalRevenue).toFixed(2)}</div>
              </Card>
              <Card className="p-4">
                <div className="text-xs uppercase tracking-wider text-slate-400">Invoices</div>
                <div className="mt-2 text-2xl font-semibold text-white">{sales.totalInvoices}</div>
              </Card>
            </div>
          ) : (
      ...
      {canStock && shopId ? (
        <div className="grid gap-4 lg:grid-cols-2">
          <Card className="p-5">
            <div className="text-sm font-semibold text-white">Transfer summary</div>
            <div className="mt-4 grid grid-cols-2 gap-3">
              <Card className="p-4">
                <div className="text-xs uppercase tracking-wider text-slate-400">Sent</div>
                <div className="mt-2 text-2xl font-semibold text-white">{transferSummary?.transfersSent ?? '—'}</div>
              </Card>
              <Card className="p-4">
                <div className="text-xs uppercase tracking-wider text-slate-400">Received</div>
                <div className="mt-2 text-2xl font-semibold text-white">{transferSummary?.transfersReceived ?? '—'}</div>
              </Card>
            </div>
          </Card>
          ...
          <Card className="overflow-hidden">
            <div className="p-5">
              <div className="text-sm font-semibold text-white">Stock summary</div>
              <div className="mt-1 text-xs text-slate-400">Current stock by item ID.</div>
            </div>
            {stockSummary !== null && stockSummary.length > 0 ? (
              <TableWrap>
                <Table>
                  ...
                  <tbody>
                    {stockSummary.map((r) => (
                      <tr key={r.itemId} className="border-t border-white/10 hover:bg-white/[0.04]">
                        <Td className="font-medium">{r.itemId}</Td>
                        <Td className="text-slate-300">{r.currentStock}</Td>
                      </tr>
                    ))}
                  </tbody>
                </Table>
      ...
      {isAdmin ? (
        <div className="grid gap-4 lg:grid-cols-2">
          <Card className="p-5">
            <div className="text-sm font-semibold text-white">Admin — Global revenue</div>
            <div className="mt-4 grid grid-cols-2 gap-3">
              <Card className="p-4">
                <div className="text-xs uppercase tracking-wider text-slate-400">Total revenue</div>
          ...
          <Card className="overflow-hidden">
            <div className="p-5">
              <div className="text-sm font-semibold text-white">Revenue by shop</div>
              <div className="mt-1 text-xs text-slate-400">Top-level view for admins.</div>
            </div>
            {revenueByShop !== null && revenueByShop.length > 0 ? (
              <TableWrap>
                <Table>
                  ...
                  <tbody>
                    {revenueByShop.map((r) => (
                      <tr key={r.shopId} className="border-t border-white/10 hover:bg-white/[0.04]">
                        <Td className="font-medium">{r.shopId}</Td>
                        <Td className="text-slate-300">₹{Number(r.revenue).toFixed(2)}</Td>
                      </tr>
                    ))}
                  </tbody>
                </Table>
              ...
              <div className="px-5 pb-5 text-sm text-slate-300">No data.</div>
            )}
          </Card>
        </div>
      ) : null}
